...
exports.isLoggedIn = function (req, res, done) {
  ...
  if (!decodedId) {
    return res.status(404).json({ err: 'Could not locate user, please try again!'})
  } else {
    userHelper.decode(decodedId).then((str) => {
      userId = str;
      User.findById(userId).exec(function(err, user) {
        if (err || !user) {
          return res.status(401).json({ err: 'There was a problem with your login credentials. Please sign in again!' });
        }
        req.user = user._id;
